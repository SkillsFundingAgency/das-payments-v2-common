...
namespace SFA.DAS.Payments.Common.Model.Core.Factories
{
    public static class CollectionPeriodFactory
    {
        ...
        public static CollectionPeriod CreateFromAcademicYearAndPeriod(short academicYear, byte period)
        {
            ...
            if (previousAcademicYear < 2016 || currentAcademicYear > 2099 ||
                currentAcademicYear - previousAcademicYear != 1)
                throw new ArgumentException(nameof(academicYear),
                    $"Invalid academic year: '{academicYear}'.  Please ensure that the academic year is in the correct format e.g. '1819'");
