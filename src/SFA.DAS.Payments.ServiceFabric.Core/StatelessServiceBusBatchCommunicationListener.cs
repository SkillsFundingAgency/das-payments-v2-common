...
namespace SFA.DAS.Payments.ServiceFabric.Core
{
    ...
    public class StatelessServiceBusBatchCommunicationListener : IStatelessServiceBusBatchCommunicationListener
    {
        ...
        public StatelessServiceBusBatchCommunicationListener(string connectionString, string endpointName,
            string errorQueueName, IPaymentLogger logger,
            IContainerScopeFactory scopeFactory, ITelemetry telemetry, IMessageDeserializer messageDeserializer,
            IApplicationMessageModifier messageModifier)
        {
            this.logger = logger ?? throw new ArgumentNullException(nameof(logger));
            this.scopeFactory = scopeFactory ?? throw new ArgumentNullException(nameof(scopeFactory));
            this.telemetry = telemetry ?? throw new ArgumentNullException(nameof(telemetry));
            this.messageDeserializer =
                messageDeserializer ?? throw new ArgumentNullException(nameof(messageDeserializer));
        ...
        private async Task EnsureSubscriptions(string endpointName, CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogInfo(
                    $"The message queue entity already exists: {ex.Message}. This could be because another instance of the service has already ensured the entity exists");
            }
            catch (Exception e)
            {
        ...
        private async Task<IList<RuleDescription>> GetExistingRules(string subscriptionName,
        ...
        private async Task<SubscriptionDescription> GetOrCreateSubscription(string endpointName,
            CancellationToken cancellationToken)
        {
            var manageClient = new ManagementClient(connectionString);
        ...
        private async Task<List<(object Message, BatchMessageReceiver Receiver, Message ReceivedMessage)>>
            ReceiveMessages(BatchMessageReceiver messageReceiver, CancellationToken cancellationToken)
        {
            var applicationMessages =
                new List<(object Message, BatchMessageReceiver Receiver, Message ReceivedMessage)>();
            ...
            try
            {
                while (!cancellationToken.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        var groupedMessages =
                            new Dictionary<Type, List<(object Message, BatchMessageReceiver MessageReceiver, Message
                                ReceivedMessage)>>();
                        foreach (var message in messages)
                        {
                            cancellationToken.ThrowIfCancellationRequested();
                            ...
                            var applicationMessages = groupedMessages.ContainsKey(key)
                                ? groupedMessages[key]
                                : groupedMessages[key] =
                                    new List<(object Message, BatchMessageReceiver MessageReceiver, Message
                                        ReceivedMessage)>();
        ...
        protected async Task ProcessMessages(Type groupType,
            ...
            CancellationToken cancellationToken)
        {
                ...
                using (var containerScope = scopeFactory.CreateScope())
                {
                            ...
                            out var handler))
                    {
                        ...
                        await Task.WhenAll(messages.Select(message =>
                            message.MessageReceiver.DeadLetter(message.ReceivedMessage)));
                        return;
                    }
                    ...
                    if (methodInfo == null)
                        throw new InvalidOperationException(
                            $"Handle method not found on handler: {handler.GetType().Name} for message type: {groupType.FullName}");
                    ...
                    await Task.WhenAll(messages.GroupBy(msg => msg.MessageReceiver).Select(group =>
                            group.Key.Complete(group.Select(msg => msg.ReceivedMessage.SystemProperties.LockToken))))
                        .ConfigureAwait(false);
                }
            }
            catch (Exception e)
            {
                logger.LogError(
                    $"Error in StatelessServiceBusBatchCommunicationListener, Message Type: {messages.First().Message.GetType().Name}, Message Count: {messages.Count}, Error: {e.Message}",
                    e);
                await Task.WhenAll(messages.Where(msg => msg.ReceivedMessage.SystemProperties.DeliveryCount < 10)
                        .GroupBy(msg => msg.MessageReceiver).Select(group =>
                            group.Key.Abandon(group.Select(msg => msg.ReceivedMessage.SystemProperties.LockToken)
                                .ToList())))
                    .ConfigureAwait(false);
                await RetryFailedMessages(groupType,
                    messages.Where(msg => msg.ReceivedMessage.SystemProperties.DeliveryCount >= 10).ToList(),
                    cancellationToken);
            }
        }
            ...
            foreach (var retryMessage in messages)
            {
                try
                {
                    using (var scope = scopeFactory.CreateScope())
                    {
                        ...
                        await retryMessage.MessageReceiver.Complete(retryMessage.ReceivedMessage.SystemProperties
                            .LockToken);
                    }
                }
                catch (Exception e)
                {
                    logger.LogError(
                        $"Error in StatelessServiceBusBatchCommunicationListener, Message Type:  {retryMessage.GetType().Name}, Error: {e.Message}.  ASB Message id: {retryMessage.ReceivedMessage.MessageId}, Message label: {retryMessage.ReceivedMessage.Label}.",
                        e);
        ...
        private async Task EnsureQueue(string queuePath)
        {
            ...
            {
                logger.LogInfo(
                    $"Queue already exists: {ex.Message}. This could be because another instance of the service has already ensured the queue exists");
            }
            catch (Exception e)
            {
